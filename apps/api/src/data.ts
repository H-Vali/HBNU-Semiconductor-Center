...
export const equipment = equipmentRows.map((item, index) => ({
  id: `eq-${index + 1}`,
  name: item.name.trim(),
  model: item.model,
  ...
  imageUrl: item.group === 'process' ? `${processImage}&ixid=process-${index}` : `${metrologyImage}&ixid=metrology-${index}`,
  features: ['예약 캘린더', '교육 인증', '사용 로그'],
  usageConditions: '교육 이수 후 담당자 승인 시 사용 가능',
  status: 'available',
